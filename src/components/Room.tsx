...
export default function Room({ code, onLeave }: { code: string; onLeave(): void }) {
  ...
  const tabs: { id: Tab; label: string; badge?: number }[] = [
    { id: 'chat', label: 'Chat', badge: unread || undefined },
    { id: 'queue', label: 'Cola', badge: room.state?.queue.length || undefined },
    { id: 'people', label: 'Gente', badge: room.participants.length },
  ];
  ...
  return (
    <div className="room">
      <header className="topbar">
        ...
        <span className={`conn-dot ${room.status}`} title={STATUS_TEXT[room.status]} />
